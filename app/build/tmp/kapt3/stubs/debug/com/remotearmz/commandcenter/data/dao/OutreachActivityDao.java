...
public abstract interface OutreachActivityDao {
    ...
    @org.jetbrains.annotations.NotNull()
    @androidx.room.Query(value = "SELECT * FROM outreach_activities WHERE followUpRequired = 1 AND (followUpDate IS NULL OR followUpDate <= :currentTime) ORDER BY followUpDate ASC NULLS LAST")
    ...
    public abstract kotlinx.coroutines.flow.Flow<java.util.Map<com.remotearmz.commandcenter.data.model.OutreachType, java.lang.Integer>> getOutreachCountByType();
    
    @org.jetbrains.annotations.NotNull()
    @androidx.room.Query(value = "SELECT outcome, COUNT(*) as count FROM outreach_activities GROUP BY outcome")
    public abstract kotlinx.coroutines.flow.Flow<java.util.Map<com.remotearmz.commandcenter.data.model.OutreachOutcome, java.lang.Integer>> getOutreachCountByOutcome();
